...
let ftm_with_moratoriums = {};

{
  function run_simulation(input_params, moratoriums) {
    let model = new Model(input_params, moratoriums);
  ...
  class Model {
    constructor(input_params, moratoriums) {
    ...
    run_simulation() {
      ...
      while (t < this.consts.t_end) {
        ...
        Model.automate_tasks(state, this.consts, this.moratoriums, states);
    ...
    static process_params(input_params) {
      ...
      consts.rnd.automation_training_flops = this.process_automation_costs(
        consts.training.full_automation_requirements /
          consts.training.goods_vs_rnd_requirements,
        consts.training.flop_gap,
      ...
      consts.rnd.automation_runtime_flops = this.process_automation_costs(
        (consts.runtime.full_automation_requirements * seconds_per_year) /
          consts.runtime.goods_vs_rnd_requirements,
        consts.runtime.flop_gap,
        consts.rnd.n_labour_tasks,
        consts.runtime.steepness
      );
      ...
      consts.initial.hardware =
        consts.initial.hardware_production *
        consts.initial.ratio_hardware_to_initial_hardware_production;
      consts.investment_rate = 0.2;

      consts.software_rnd.experiments_task_weights = odds_to_probs(
        input_params.research_to_compute_experiments_task_weight_ratio
      );

      let frac_gwp_hardware_rnd_2020 = 0.2e-2;
      let frac_gwp_software_rnd_2020 = 0.02e-2;
      consts.initial.rnd_input_hardware =
        consts.initial.gwp * frac_gwp_hardware_rnd_2020;
      consts.initial.rnd_input_software =
        consts.initial.gwp * frac_gwp_software_rnd_2020;

      consts.initial.goods.share.capital =
        1 - consts.initial.goods.share.cognitive;
      consts.initial.hardware_rnd.share.capital =
        1 - consts.initial.hardware_rnd.share.cognitive;
      consts.initial.software_rnd.share.cognitive =
        1 - consts.initial.software_rnd.share.experiments;

      consts.initial.hardware_rnd.share.compute =
        consts.initial.rnd.share.compute;
      consts.initial.software_rnd.share.compute =
        consts.initial.rnd.share.compute;

      let share = consts.initial.goods;
      for (let share of [
        consts.initial.goods.share,
        consts.initial.hardware_rnd.share,
        consts.initial.software_rnd.share,
      ]) {
      ...
      consts.hardware_rnd.capital_substitution =
        consts.rnd.capital_substitution;
      consts.hardware_rnd.labour_substitution = consts.rnd.labour_substitution;

      consts.software_rnd.capital_substitution =
        consts.software_rnd.experiments_substitution;
      consts.software_rnd.labour_substitution = consts.rnd.labour_substitution;

      consts.hardware_delay_idx = nj.even_round(
        consts.hardware_delay / consts.t_step
      );
    ...
    static init_state_and_consts(state, consts) {
      ...
      state.hardware_performance.cumulative_rnd_input = nj.div(
        ...
        nj.mult(
          consts.initial.ratio_initial_to_cumulative_input_hardware_rnd,
          consts.rnd.parallelization_penalty
        )
      );
      ...
      state.software.cumulative_rnd_input = nj.div(
        ...
        nj.mult(
          consts.initial.ratio_initial_to_cumulative_input_software_rnd,
          consts.rnd.parallelization_penalty
        )
      );
      ...
      state.capital =
        (initial.gwp * consts.investment_rate) /
        (nj.exp(initial.capital_growth) - 1);

      state.hardware = initial.hardware;

      state.compute = initial.hardware * initial.software;
      state.compute_investment =
        (initial.hardware_production * consts.t_step) /
        initial.buyable_hardware_performance;

      state.goods.tfp = initial.tfp_goods;
      state.rnd.tfp = initial.tfp_rnd;

      state.money_spent_training =
        initial.biggest_training_run /
        (initial.software * initial.buyable_hardware_performance);

      //
      // Initialize fractional inputs

      // Split of gwp between capital and compute
      state.frac_gwp.compute.v =
        state.compute_investment / initial.gwp / consts.t_step;
      ...
      state.frac_labour.goods.v =
        1 - state.frac_labour.hardware_rnd.v - state.frac_labour.software_rnd.v;
      ...
      state.frac_compute.training.v =
        initial.biggest_training_run / state.compute;
      state.frac_compute.goods.v =
        1 -
        state.frac_compute.hardware_rnd.v -
      ...
      if (initial.biggest_training_run > state.compute) {
        throw new Error(
          "Initial biggest training run is bigger than available compute"
        );
      }
      ...
      let _initialize_task_weight = (
        ...
        if (capital == null)
          capital = state.capital * state.frac_capital[item].v;
        if (labour == null) labour = state.labour * state.frac_labour[item].v;
        if (compute == null)
          compute = state.compute * state.frac_compute[item].v;

        let no_automation_labour_task_input = nj.zeros(
          consts[category].n_labour_tasks + 1
        );
        no_automation_labour_task_input.fill(
          labour / consts[category].n_labour_tasks,
          1
        );

        let no_automation_compute_task_input = nj.zeros(
          consts[category].n_labour_tasks + 1
        );
        no_automation_compute_task_input[0] = compute;

        let share = initial[item].share;
        let initial_capital_to_cognitive_share_ratio =
          share.capital / share.cognitive;
        let initial_compute_to_labour_share_ratio =
          share.compute / share.labour;

        let task_weights = this.adjust_task_weights(
          capital,
          no_automation_labour_task_input,
          no_automation_compute_task_input,
          this.get_task_compute_to_labour_ratio(
            consts[category].automation_runtime_flops,
            consts[category].automation_training_flops,
            consts.initial.biggest_training_run,
            consts,
            category
          ),
      ...
      let experiments_compute =
        state.hardware ** consts.software_rnd.experiments_efficiency;
      consts.initial.software_rnd.share.capital =
        consts.initial.software_rnd.share.experiments;
      _initialize_task_weight(
        state,
        consts,
        "rnd",
        "software_rnd",
    ...
    static process_automation_costs(
      full_automation_flops,
      flop_gap,
      n_labour_tasks,
      steepness
    ) {
      let automation_flops = this.quantiles_from_gap(
        full_automation_flops,
        flop_gap
      );
      automation_flops = this.interpolate_quantiles(
        automation_flops,
        n_labour_tasks
      );
      automation_flops = this.add_steepness(
        full_automation_flops,
        flop_gap,
        automation_flops,
        steepness
        ...
        if (prev_quantile == next_quantile) {
          ...
          value =
            ...
            (next_value / prev_value) **
              ((q - prev_quantile) / (next_quantile - prev_quantile));
        }
      ...
      */
      ...
      for (let i = 0; i < result.length; i++) {
        let r =
          ...
          (nj.log10(gap_low) +
            Math.ceil(
              (nj.log10(requirements[i]) - nj.log10(gap_low)) / steepness
            ) *
              steepness);
    ...
    static adjust_task_weights(
      ...
      */
      ...
      let task_input = nj.add(
        labour_task_input,
        nj.mult(task_compute_to_labour_ratio, compute_task_input)
      );

      let labour_share = nj.sum(
        nj.mult(labour_task_input, nj.pow(task_input, labour_substitution - 1))
      );
      let compute_share = nj.sum(
        nj.mult(
          task_compute_to_labour_ratio,
          nj.mult(
            compute_task_input,
            nj.pow(task_input, labour_substitution - 1)
          )
        )
      );

      let compute_to_labour_task_weight_ratio =
        (compute_to_labour_share_ratio * labour_share) / compute_share;

      let [compute_task_weight, labour_task_weight] = odds_to_probs(
      ...
      let inner_task_weights = nj.array(
        [compute_task_weight].concat(
          Array(n_labour_tasks).fill(labour_task_weight)
        )
      );
      ...
      let capital_to_cognitive_task_weight_ratio =
        (capital_to_cognitive_share_ratio * cognitive_share) / capital_share;

      let [capital_task_weight, cognitive_task_weight] = odds_to_probs(
        capital_to_cognitive_task_weight_ratio
      );

      let outer_task_weights = nj.array([
        capital_task_weight,
        cognitive_task_weight,
      ]);
    ...
    static update_rnd_state(state, consts, states) {
      function _update_rnd(
        ...
        let adjusted_input =
          research_input ** consts.rnd.parallelization_penalty;
        let new_cumulative_adjusted_input =
          cumulative_adjusted_input + adjusted_input * consts.t_step;
        let growth_in_cumulative_inputs =
          new_cumulative_adjusted_input / cumulative_adjusted_input;
        let ceiling_penalty =
          (nj.log10(performance_ceiling) - nj.log10(current_performance)) /
          (nj.log10(performance_ceiling) - nj.log10(initial_performance));
        let performance_growth_rate =
          growth_in_cumulative_inputs ** (returns * ceiling_penalty);
        let new_performance = Math.min(
      ...
      if (state.t_idx == 1) {
        ...
        let initial_hardware_improvement_rate =
          improved_hardware_performance /
          consts.initial.buyable_hardware_performance;

        consts.initial.hardware_performance =
          consts.initial.buyable_hardware_performance *
          initial_hardware_improvement_rate ** consts.hardware_delay_idx;
      ...
      state.hardware_performance.cumulative_rnd_input =
        hardware_result.new_cumulative_adjusted_input;
      ...
      state.software.cumulative_rnd_input =
        software_result.new_cumulative_adjusted_input;
      state.software_rnd.ceiling_penalty = software_result.ceiling_penalty;
    }

    static update_rampup(state, consts) {
      state.rampup =
        this.get_frac_tasks_automated(state.goods, consts.goods) >=
        consts.rampup_trigger;
    }
    ...
    static allocate_frac_inputs(state, consts, states) {
      function _update_frac_input(x, state, consts) {
        let frac =
          x.v *
          nj.exp(consts.t_step * (state.rampup ? x.growth_rampup : x.growth));
      ...
      if (
        state.money_spent_training > consts.money_cap_training_before_wakeup &&
        !states[state.t_idx - 1].rampup
      ) {
        state.frac_compute.training.v =
          states[state.t_idx - 1].frac_compute.training.v;
      }

      state.frac_capital.goods.v =
        1 -
        state.frac_capital.hardware_rnd.v -
        state.frac_capital.software_rnd.v;
      state.frac_labour.goods.v =
        1 - state.frac_labour.hardware_rnd.v - state.frac_labour.software_rnd.v;
      state.frac_compute.goods.v =
        1 -
    ...
    static calculate_total_inputs(state, consts, states) {
      // Compute inputs
      state.compute_investment =
        state.gwp * state.frac_gwp.compute.v * consts.t_step;

      let t_diff = state.t_idx - consts.hardware_delay_idx;
      if (t_diff >= 0) {
        state.buyable_hardware_performance =
          states[t_diff].hardware_performance.v;
      } else {
        state.buyable_hardware_performance =
          states[0].hardware_performance.v *
          (states[1].hardware_performance.v /
            states[0].hardware_performance.v) **
            t_diff;
      }

      let new_hardware =
        state.compute_investment * state.buyable_hardware_performance;

      state.hardware =
        state.hardware * (1 - consts.compute_depreciation) ** consts.t_step +
        new_hardware;
      state.compute = state.hardware * state.software.v;

      // Non compute inputs
      let capital_investment =
        state.gwp * consts.investment_rate * consts.t_step;
      state.capital = state.capital + capital_investment;

      state.labour =
        state.labour * nj.exp(consts.labour_growth * consts.t_step);
    }
    ...
    static calculate_money_spent_training(state, consts) {
      state.money_spent_training =
        (state.compute * state.frac_compute.training.v) /
        (state.software.v * state.buyable_hardware_performance);
    }
    ...
    static automate_tasks(state, consts, moratoriums, states) {
      // THIS IS NEW
      let isMoratorium = false;
      for (var i = 0; i < moratoriums.length; i++) {
        const start = moratoriums[i][0];
        const end = moratoriums[i][1];
        if (state.t_year >= start && state.t_year <= end) {
          isMoratorium = true;
          break;
        }
      }
      // isMoratorium = false;
      if (!isMoratorium) {
        state.biggest_training_run =
          state.compute * state.frac_compute.training.v;
        console.log("not moratorium");
      } else {
        state.biggest_training_run =
          states[states.length - 1].biggest_training_run;
        console.log(state.biggest_training_run);
        console.log(states.length);
      }
      // ^ THIS IS NEW
      let _update_automatable_tasks = (s, p, category) => {
        s.task_compute_to_labour_ratio = this.get_task_compute_to_labour_ratio(
          p.automation_runtime_flops,
          p.automation_training_flops,
          state.biggest_training_run,
          consts,
          category
        );
        s.automatable_tasks = nj.count_true(
          nj.lt(
            p.automation_training_flops,
            nj.mult(
              state.biggest_training_run,
              consts.runtime_training_max_tradeoff
            )
          )
        );
      };
      ...
      state.hardware_rnd.task_compute_to_labour_ratio =
        state.rnd.task_compute_to_labour_ratio; // TODO hack
      state.software_rnd.task_compute_to_labour_ratio =
        state.rnd.task_compute_to_labour_ratio; // TODO hack
    }

    static get_task_compute_to_labour_ratio(
      automation_runtime_flops,
      automation_training_flops,
      biggest_training_run,
      consts,
      category
    ) {
      let runtime_requirements = nj.mult(
        automation_runtime_flops,
        nj.pow(
          nj.div(automation_training_flops, biggest_training_run),
          consts.runtime_training_tradeoff
        )
      );
    ...
    static production(state, consts, states) {
      ...
      state.software_rnd.experiments_compute =
        state.hardware ** consts.software_rnd.experiments_efficiency;
      this.produce(
        state,
        consts,
    ...
    static produce(
      ...
      item,
      capital = null,
      apply_tfp_to_cognitive_outputs = false
    ) {
      ...
      state[item].task_input = nj.add(
        state[item].labour_task_input,
        nj.mult(
          state[category].task_compute_to_labour_ratio,
          state[item].compute_task_input
        )
      );
    ...
    static post_production(state, consts, states) {
      ...
      let {
        labour: no_automation_labour_task_input_rnd,
        compute: no_automation_compute_task_input_rnd,
      } = this.solve_allocation(
      ...
      let no_automation_task_input_hardware_rnd = nj.add(
      ...
      let no_automation_output = this.nested_ces_production_function(
        state.hardware_rnd.capital,
        no_automation_task_input_hardware_rnd,
        consts.hardware_rnd.capital_task_weights,
        consts.hardware_rnd.labour_task_weights,
        consts.rnd.capital_substitution,
        consts.rnd.labour_substitution,
        state.rnd.tfp
      ...
      state.hardware_rnd.automation_multiplier =
        state.hardware_rnd.output / no_automation_output;

      // Compute inputs to hardware R&D
      let rnd_input_to_hardware_performance_investment_factor =
        consts.initial.rnd_input_hardware / states[0].hardware_rnd.output;
      state.hardware_performance.rnd_input =
        state.hardware_rnd.output *
        rnd_input_to_hardware_performance_investment_factor;

      // Compute inputs to sotware R&D
      let rnd_input_to_software_investment_factor =
        consts.initial.rnd_input_software / states[0].software_rnd.output;
      state.software.rnd_input =
        state.software_rnd.output * rnd_input_to_software_investment_factor;
    }
    ...
    static ces_production_function(inputs, alphas, rho, tfp = 1) {
      let result =
        tfp *
        nj.sum(nj.div(nj.mult(alphas, nj.pow(inputs, rho)), nj.sum(alphas))) **
          (1 / rho);
      return result;
    }
    ...
    static solve_allocation(L, C, beta, rho, eta, AT) {
      ...
      */
      ...
      if (!found_critical_index) {
        ...
        for (let i = 0; i < I + 1; i++) {
          compute_input_task[i] =
            (C * beta[i] ** sigma * eta[i] ** (sigma - 1)) / Z;
        }
    ...
    static compute_shares(
      ...
      let task_input = nj.add(
        labour_task_input,
        nj.mult(compute_task_input, task_compute_to_labour_ratio)
      );
      ...
      let cognitive_share =
        cognitive_task_weight * cognitive_input ** capital_substitution;
    ...
    static get_automated_tasks_count(statex) {
      let count =
        nj.count_true(
          nj.gt(
            nj.mult(
              statex.task_compute_to_labour_ratio,
              statex.compute_task_input
            ),
    ...
    static get_frac_automatable_tasks(
      state,
      consts,
      category,
      with_tradeoff = true
      ...
      return (
        this.get_automatable_task_count(
          state,
          consts,
          category,
          with_tradeoff
        ) / consts[category].n_labour_tasks
      );
    }

    static get_automatable_task_count(
      state,
      consts,
      category,
      with_tradeoff = true
      ...
      return (
        nj.count_true(
          nj.lt(
            consts[category].automation_training_flops,
            state.biggest_training_run *
              (with_tradeoff ? consts.runtime_training_max_tradeoff : 1)
          )
        ) - 1
      );
    }
    ...
    post_simulation() {
      ...
      for (let i = 0; i < this.states.length; i++) {
        ...
        if (
          rampup_mid == null &&
          prev_state &&
          Model.get_frac_tasks_automated(prev_state.goods, this.consts.goods) >=
            0.2
        ) {
          rampup_mid = year;
        }
        ...
        if (
          sub_agi_year == null &&
          prev_state &&
          Model.get_frac_automatable_tasks(
            prev_state,
            this.consts,
            "goods",
            false
      ...
      let doubling_times = [
        this.consts.t_step / Math.log2(this.states[1].gwp / this.states[0].gwp),
      ];
      if (rampup_start != null) {
        ...
        for (let i = reference_idx; i < this.states.length; i++) {
          if (this.states[i].gwp > 2 * this.states[reference_idx].gwp) {
            doubling_times.push(
              this.index_to_time(i) - this.index_to_time(reference_idx)
            );
            reference_idx = i;
          }
        }
      ...
      for (let i = 0; i < this.states.length; i++) {
        frac_tasks_automated_goods[i] = Model.get_frac_tasks_automated(
          this.states[i].goods,
          this.consts.goods
        );
      }
      ...
      for (let i = 0; i < this.states.length; i++) {
        frac_tasks_automated_rnd[i] = Model.get_frac_tasks_automated(
          this.states[i].hardware_rnd,
          this.consts.rnd
        );
      }
    ...
    get_growth(path_or_thread) {
      let thread =
        typeof path_or_thread == "string"
          ? this.get_thread(path_or_thread)
          : path_or_thread;
      ...
      for (let i = steps_per_year; i < this.states.length; i++) {
        ...
        growth[i - steps_per_year] = Math.log(
          thread[i] / thread[i - steps_per_year]
        );
      }
    ...
    get_timeline_metrics() {
      ...
      timeline_metrics["automation_gns_20%"] = this.time_when(
        nj.gte(this.frac_tasks_automated_goods, 0.2)
      );
      timeline_metrics["automation_gns_100%"] = this.time_when(
        nj.gte(this.frac_tasks_automated_goods, 1.0)
      );

      timeline_metrics["sub_agi_year"] = this.sub_agi_year;
      timeline_metrics["agi_year"] = this.agi_year;

      timeline_metrics["automation_rnd_20%"] = this.time_when(
        nj.gte(this.frac_tasks_automated_rnd, 0.2)
      );
      timeline_metrics["automation_rnd_100%"] = this.time_when(
        nj.gte(this.frac_tasks_automated_rnd, 1.0)
      );
    ...
    get_takeoff_metrics() {
      ...
      let rnd_automation_multiplier = this.get_thread(
        "hardware_rnd.automation_multiplier"
      );
      takeoff_metrics["cog_output_multiplier"] = this.length_between_thresholds(
        nj.gt(rnd_automation_multiplier, 2),
        nj.gt(rnd_automation_multiplier, 10)
      );

      // Time from powerful sub-AGI to AGI
      takeoff_metrics["sub_agi_to_agi"] =
        this.agi_year != null ? this.agi_year - this.sub_agi_year : nj.nan;
      ...
      for (let i = 0; i < this.states.length - steps_per_year; i++) {
        gwp_growth[i] = Math.log(
          this.states[i + steps_per_year].gwp / this.states[i].gwp
        );
      }
      takeoff_metrics["gwp_growth"] = this.length_between_thresholds(
        nj.gt(gwp_growth, 0.05),
        nj.gt(gwp_growth, 0.2)
      );
    ...
    get_summary_table() {
      ...
      let prerampup =
        this.rampup_start == null
          ? null
          : (this.consts.t_start + this.rampup_start) / 2;
      ...
      let doubling_time_metrics = {
        ...
        cumulative_rnd_input_software: this.get_thread(
          "software.cumulative_rnd_input"
        ),
      };

      for (let [period, t] of Object.entries({
        "Pre wake-up": prerampup,
        "Wake-up": this.rampup_start,
        "Mid rampup": this.rampup_mid,
        "Full economic automation":
          this.timeline_metrics["automation_gns_100%"],
      })) {
        if (t == null) {
          let summary_row = {
            period: period,
        ...
        // Auxiliary functions
        let dt = (s) =>
          Math.log2(s[idx_end] / s[idx]) != 0
            ? 1 / Math.log2(s[idx_end] / s[idx])
            : nj.nan;
        ...
        for (let doubling_time_metric in doubling_time_metrics) {
          summary_row[`${doubling_time_metric} growth rate`] = gr(
            doubling_time_metrics[doubling_time_metric]
          );
          summary_row[`${doubling_time_metric} doubling time`] = dt(
            doubling_time_metrics[doubling_time_metric]
          );
        }
  ...
  class SimulationState {
    constructor(consts) {
      ...
      this.frac_capital = SimulationState.create_frac_state([
        "goods",
        "hardware_rnd",
        "software_rnd",
      ]);
      this.frac_labour = SimulationState.create_frac_state([
        "goods",
        "hardware_rnd",
        "software_rnd",
      ]);
      this.frac_compute = SimulationState.create_frac_state([
        "goods",
        "hardware_rnd",
        "software_rnd",
        "training",
      ...
      this.goods.task_compute_to_labour_ratio = nj.zeros(
        consts.goods.n_labour_tasks + 1
      );
      ...
      this.rnd.task_compute_to_labour_ratio = nj.zeros(
        consts.rnd.n_labour_tasks + 1
      );

      this.frac_capital.hardware_rnd = cheap_deep_copy(
        consts.frac_capital.hardware_rnd
      );

      this.frac_labour.hardware_rnd = cheap_deep_copy(
        consts.frac_labour.hardware_rnd
      );
      this.frac_labour.software_rnd = cheap_deep_copy(
        consts.frac_labour.software_rnd
      );

      this.frac_compute.hardware_rnd = cheap_deep_copy(
        consts.frac_compute.hardware_rnd
      );
      this.frac_compute.software_rnd = cheap_deep_copy(
        consts.frac_compute.software_rnd
      );
      this.frac_compute.training = cheap_deep_copy(
        consts.frac_compute.training
      );
  ...
  function run_bioanchors_model({
    ...
    let fast_training_investment_growth = nj.pow(
      2,
      1 / fast_training_investment_doubling_time
    );

    let timesteps = nj.arange(t_start, t_end, t_step);

    let slow_training_investment_start_index = nj.round(
      (t_initial + fast_training_investment_duration) / t_step - t_start
    );

    let fast_training_investment = nj.mult(
      initial_training_investment,
      nj.pow(
        fast_training_investment_growth,
        nj.sub(
          timesteps.slice(0, slow_training_investment_start_index),
          t_initial
        )
      )
    );
    let slow_training_investment = nj.mult(
      ...
      nj.pow(
        slow_training_investment_growth,
        nj.sub(
          timesteps.slice(slow_training_investment_start_index),
          timesteps[slow_training_investment_start_index - 1]
        )
      )
    );
    let training_investment = nj.concatenate(
      fast_training_investment,
      slow_training_investment
    );

    let hardware = nj.mult(
      initial_hardware,
      nj.pow(hardware_growth, nj.sub(timesteps, t_initial))
    );
    ...
    let software = nj.mult(
      ...
      nj.pow(
        software_growth,
        nj.sub(software_timesteps, software_doubling_start)
      )
    );
  ...
  ftm_with_moratoriums.run_simulation = run_simulation;
  ftm_with_moratoriums.run_bioanchors_model = run_bioanchors_model;
  ftm_with_moratoriums.Model = Model;
}

module.exports = ftm_with_moratoriums;
